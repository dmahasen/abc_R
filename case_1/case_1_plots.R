...
par(mfrow=c(3,4),mar=c(4,2,2,2)+0.1,cex=1.75)
hist(ABC_1_res[,"S_0"],xlab=expression('initial susceptibles, S(0)'),main="",col=cols[1],xlim=S_0_xlim,freq=F,ylim=S_0_ylim,breaks=breaks_S0,cex=1.5)
abline(v=99,col=4,lty=2,lwd=2)
hist(ABC_1_res[,"beta"],xlab=expression(paste('transmission rate, ',beta)),main='',col=cols[1],xlim=beta_xlim,freq=F,ylim=beta_ylim,breaks=breaks_beta)
abline(v=1.5,col=4,lty=2,lwd=2)
hist(ABC_1_res[,"gamma"],xlab=expression(paste('recovery rate, ',gamma)),main='',col=cols[1],xlim=gamma_xlim,freq=F,ylim=gamma_ylim,breaks=breaks_gamma)
abline(v=0.5,col=4,lty=2,lwd=2)
hist(ABC_1_res[,"beta"]/ABC_1_res[,"gamma"],xlab=expression(R[0]),main='',col=cols[1],xlim=R0_xlim,freq=F,ylim=R0_ylim,breaks=breaks_R0)
abline(v=3,col=4,lty=2,lwd=2)
par(mar=c(4,2,1,2)+0.1)
hist(ABC_2_res[,"S_0"],xlab=expression('initial susceptibles, S(0)'),main='',col=cols[2],xlim=S_0_xlim,freq=F,ylim=S_0_ylim)
abline(v=99,col=4,lty=2,lwd=2)
hist(ABC_2_res[,"beta"],xlab=expression(paste('transmission rate, ',beta)),main='',col=cols[2],xlim=beta_xlim,freq=F,ylim=beta_ylim)
abline(v=1.5,col=4,lty=2,lwd=2)
hist(ABC_2_res[,"gamma"],xlab=expression(paste('recovery rate, ',gamma)),main='',col=cols[2],xlim=gamma_xlim,freq=F,ylim=gamma_ylim)
abline(v=0.5,col=4,lty=2,lwd=2)
hist(ABC_2_res[,"beta"]/ABC_2_res[,"gamma"],xlab=expression(R[0]),main='',col=cols[2],xlim=R0_xlim,freq=F,ylim=R0_ylim)
abline(v=3,col=4,lty=2,lwd=2)

hist(ABC_3_res[,"S_0"],xlab=expression('initial susceptibles, S(0)'),main='',col=cols[3],xlim=S_0_xlim,freq=F,ylim=S_0_ylim,breaks=breaks_S0)
abline(v=99,col=4,lty=2,lwd=2)
hist(ABC_3_res[,"beta"],xlab=expression(paste('transmission rate, ',beta)),main='',col=cols[3],xlim=beta_xlim,freq=F,ylim=beta_ylim,breaks=breaks_beta)
abline(v=1.5,col=4,lty=2,lwd=2)
hist(ABC_3_res[,"gamma"],xlab=expression(paste('recovery rate, ',gamma)),main='',col=cols[3],xlim=gamma_xlim,freq=F,ylim=gamma_ylim,breaks=breaks_gamma)
abline(v=0.5,col=4,lty=2,lwd=2)
...
png('model_run_case_1.png',width=5000,height=2200,units='px',res=300)
par(mfrow=c(1,3),cex=2,mar=c(5,4,4,1)+0.1)
plot(data[,1],pch=19,ylab='Number',xlab='Time (day)',col=2,ylim=c(0,max(data)),main='ABC-rejection 1')
legend('topleft',c('Infected','Recovered'),pch=c(19,17),col=c(2,3),bty='n',cex=0.75)
for(j in 1:10){
  i<-sample(1000,1)
